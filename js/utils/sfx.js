...
// Cute female dealer welcome chimes & speech synthesis greeting
export function playDealerWelcome() {
  if (!isSfxEnabled()) return;
  try {
    const ctx = getAudioContext();
    const now = ctx.currentTime;

    // Cute, high-pitched magical chime/giggle melody: C6 -> E6 -> G6 -> C7 -> A6 -> C7
    ...
    // Speak welcome message using Web Speech API SpeechSynthesis if available
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel();
      const text = "Halo sayang! Selamat datang di meja keberuntunganmu. Aku Eshabela, bandar cantikmu hari ini. Semoga kartu kartumu hoki ya! Yuk, kita mulai pertandingannya! Semangat!";
      ...
      const speakNow = () => {
        ...
        // Try: Indonesian female → any Indonesian → any female → default
        const indFemale = voices.find(v => v.lang.startsWith('id') && v.name.toLowerCase().match(/female|wanita|perempuan|woman/));
        const indVoice  = voices.find(v => v.lang.startsWith('id'));
        const anyFemale = voices.find(v => v.name.toLowerCase().match(/female|woman|girl/));
        utterance.voice = indFemale || indVoice || anyFemale || null;
    ...
    console.error('Dealer welcome sound error:', e);
  }
}
