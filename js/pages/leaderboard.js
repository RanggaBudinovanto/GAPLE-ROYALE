...
export function render(container) {
  ...
  async function renderContent() {
    ...
    content.innerHTML = `
      ...
      <div class="tabs">
        <button class="tab ${activeTab === 'global' ? 'tab--active' : ''}" data-tab="global">Global</button>
        <button class="tab ${activeTab === 'weekly' ? 'tab--active' : ''}" data-tab="weekly">Mingguan</button>
      </div>
        ...
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr style="border-bottom:1px solid var(--border-default);">
              ...
              <th style="padding:var(--sp-3);text-align:center;font-family:var(--font-display);font-size:11px;letter-spacing:0.14em;text-transform:uppercase;color:var(--text-secondary);">Menang</th>
          ...
          <tbody>
            ${pageData.length > 0 ? pageData.map(entry => `
              <tr style="border-bottom:1px solid var(--border-default);${entry.userId === user.id ? 'background:rgba(212,160,23,0.06);' : ''}">
                ...
                <td style="padding:var(--sp-3);text-align:center;font-family:var(--font-mono);font-weight:600;">${entry.wins}</td>
  ...
  function renderPodium(entry, rank, height) {
    ...
    return `
      <div style="text-align:center;animation:podiumRise 0.5s var(--ease-spring) ${rank * 0.15}s both;">
        ...
        <div style="font-family:var(--font-mono);font-size:12px;color:var(--text-secondary);margin-bottom:var(--sp-2);">${entry.wins} wins</div>
